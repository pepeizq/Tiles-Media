...
namespace Plataformas
{
    ...
    public static class DisneyPlus
    {
        ...
        {
            Objetos.botonDisneyPlusBuscar.Click += BuscarClick;
            Objetos.botonDisneyPlusBuscar.PointerEntered += Animaciones.EntraRatonBoton2;
            Objetos.botonDisneyPlusBuscar.PointerExited += Animaciones.SaleRatonBoton2;

            Objetos.tbDisneyPlusBuscar.KeyDown += BuscarPulsar;

            ApplicationDataContainer datos = ApplicationData.Current.LocalSettings;

            if (datos.Values["OpcionesDisneyPlusAPIID"] == null)
            {
                Objetos.tbOpcionesDisneyPlusAPIID.Text = "AIzaSyC2mAim7jYXCR8ePfx59BdwU8zCTTNaURs";
                datos.Values["OpcionesDisneyPlusAPIID"] = Objetos.tbOpcionesDisneyPlusAPIID.Text;
            }
            else
            {
                Objetos.tbOpcionesDisneyPlusAPIID.Text = datos.Values["OpcionesDisneyPlusAPIID"].ToString();
            }

            Objetos.tbOpcionesDisneyPlusAPIID.TextChanged += DetectarAPIID;

            if (datos.Values["OpcionesDisneyPlusSearchID"] == null)
            {
                Objetos.tbOpcionesDisneyPlusSearchID.Text = "4131afa3b274f44e2";
                datos.Values["OpcionesDisneyPlusSearchID"] = Objetos.tbOpcionesDisneyPlusSearchID.Text;
            }
            else
            {
                Objetos.tbOpcionesDisneyPlusSearchID.Text = datos.Values["OpcionesDisneyPlusSearchID"].ToString();
            }

            Objetos.tbOpcionesDisneyPlusSearchID.TextChanged += DetectarSearchID;

            Objetos.botonOpcionesDisneyPlusAPIAyuda.Click += AbrirAyudaClick;
            Objetos.botonOpcionesDisneyPlusAPIAyuda.PointerEntered += Animaciones.EntraRatonBoton2;
            Objetos.botonOpcionesDisneyPlusAPIAyuda.PointerExited += Animaciones.SaleRatonBoton2;
        }
        ...
        {
            if (Objetos.tbDisneyPlusBuscar.Text.Trim().Length > 3)
            {
                ActivarDesactivar(false);
                Objetos.prDisneyPlusResultados.Visibility = Visibility.Visible;

                ApplicationDataContainer datos = ApplicationData.Current.LocalSettings;
                Objetos.gvDisneyPlusResultados.Items.Clear();

                await Task.Delay(100);
                List<string> resultados = Google.Buscar(Objetos.tbDisneyPlusBuscar.Text.Trim(), datos.Values["OpcionesDisneyPlusAPIID"].ToString(), datos.Values["OpcionesDisneyPlusSearchID"].ToString(), "disney");
                            ...
                            if (html != null)
                            {
                                if (html.Contains("<title>") == true)
                                {
                                    ...
                                    if (añadir == true)
                                    {
                                        ...
                                        Objetos.gvDisneyPlusResultados.Items.Add(item);
                                    }
                                }
                            }
                        }
                ...
                Objetos.prDisneyPlusResultados.Visibility = Visibility.Collapsed;
            }
        }
        ...
        private static void ImagenItemClick(object sender, RoutedEventArgs e)
        {
            ...
            Tiles.PrecargarMedia(streaming.nombre,
                    streaming.enlace, "disneyplus" + azar.Next(10000).ToString(),
        ...
        {
            Objetos.botonDisneyPlusBuscar.IsEnabled = estado;
            Objetos.tbDisneyPlusBuscar.IsEnabled = estado;
        }
    }
